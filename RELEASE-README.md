...
- **键盘操作**：选中后 `Ctrl+X` 快速上传（当前目标），或 `Ctrl+Shift+X` 唤起服务器菜单（数字 `1-9/0/A-Z` 选服务器），`Enter` 确认 / `Esc` 取消
- **菜单栏**：VS Code 风格菜单（文件 / 视图 / 设置 / 帮助），带快捷键提示与当前项标记
- **主题**：自动跟随系统 / 深色 / 亮色
...
也可以纯键盘操作：点击选中文件 / 目录 / 提交 → `Ctrl+X` 快速上传，或 `Ctrl+Shift+X` 选服务器 → `Enter` 确认上传。
...
| `Ctrl+W` | 关闭项目 |
| `Ctrl+X` | 快速上传选中项 |
| `Ctrl+Shift+X` | 选择服务器上传（菜单内数字/字母选择）|
| `Esc` | 关闭弹窗 / 取消上传 |
