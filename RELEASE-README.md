...
- **键盘操作**：`Ctrl+Shift+Alt+X` 唤起服务器菜单，数字 `1-9/0/A-Z` 选服务器，`Enter` 确认 / `Esc` 取消
...
也可以纯键盘操作：点击选中文件 / 目录 / 提交 → `Ctrl+Shift+Alt+X` → 按序号或鼠标选择服务器 → `Enter` 确认上传。
...
| `Ctrl+,` | 上传目标管理 |
| `Ctrl+W` | 关闭项目 |
| `Ctrl+Shift+Alt+X` | 选择服务器上传（菜单内数字/字母选择）|
| `Esc` | 关闭弹窗 / 取消上传 |
